...
import { useAllInactiveAuctions } from "../../hooks/useAllInactiveAuctions";
...
export default function InactiveAuctions() {
  ...
  const { data: auctions = [], isValidating } = useAllInactiveAuctions(
    account,
    chainId
  );
  ...
  return (
    <Widget
      ...
      header={`Inactive Auctions : ${auctions.length}`}
      subHeaderComponent={
