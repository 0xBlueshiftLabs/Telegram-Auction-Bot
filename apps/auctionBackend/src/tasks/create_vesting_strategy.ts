...
const createVestingStrategy: () => void = () => {
  task("createVestingStrategy", "Starts a new auction").setAction(
    ...
    const TokenVestingFactory = await hardhatRuntime.ethers.getContractFactory("TokenVestingFactory");
    const tokenVestingFactory = await TokenVestingFactory.deploy(easyAuction.address, {
    ...
    console.log(
    "Strategy Address: ",tokenVestingFactory.address
    );
    },
  );
};
